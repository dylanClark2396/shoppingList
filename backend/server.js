...
app.post('/projects/:projectId/labels', requireAuth, async (req, res) => {
  try {
    ...
    const { machine, substrate, color, labelName, spaceName, size, notes, quantity } = req.body
    if (!['P-touch', 'Cricut'].includes(machine)) {
      return res.status(400).json({ error: 'Invalid machine' })
    }

    const label = { id: generateId(), machine, substrate, color, labelName, spaceName, size, notes, quantity }
